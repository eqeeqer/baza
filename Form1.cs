...
namespace WindowsFormsApplication1
{
    public partial class Form1 : Form
    {
        ...
        {
            OleDbConnection conn = new OleDbConnection(conString);//conString - строка подключения
            conn.Open();
            string oleCommand = "SELECT * FROM Часы WHERE Цена <= 500;";
            OleDbDataAdapter da = new OleDbDataAdapter(sqlCommand, conn);
            DataTable dt = new DataTable();
            da.Fill(dt);
            matr.DataSource = dt; //выводим
        }
